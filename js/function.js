...
function kurangnyawa(nl){
    game.nyawaraja-=nl;
    if (game.hiNyawa>game.hiNyawa) game.hiNyawa = game.nyawaraja;
}
...
function setPlatform(map, img, tileW, hero){
    ...
    game.charX = 10;
    game.charY = 5;
    game.screenW = Math.floor(game.lebar/(game.tileW*game.skalaSprite))+10;
...
function setPlatformEnemyraja(id, ob){
    //loop cari posisi musuh tile id = 5
    for (var i = 0; i < game.map.length; i++){
        for (var j = 0; j < game.map[0].length; j++){
            if (game.map[i][j][0] == 5 && game.map[i][j][3] == id){
                var musuh = setSprite(ob.animDiam, 34, 44);
                musuh.dataGambar = ob;
                musuh.charX = i;
                musuh.charY = j;
                musuh.posX = 0;
        ...
        if (musuh.stat<2 && game.aktif){
            if (jarak(game.karakter.x, game.karakter.y, musuh.x, musuh.y)<game.tileW){
                if (game.lompatY>0){
                    ...
                    if (game.nyawaraja !=1 && game.nyawaraja != 0) {
                        kurangnyawa(1)
                        musuh.stat = 4;
                        game.lompatY = -7;
                    } else if(game.nyawaraja == 1){
                        musuh.stat = 2;
                        game.lompatY = -5;
                        game.nyawaraja = 0
                        game.musikraja.stop()
                        mainkanSuara(dataSuara.rajamati)
                        setTimeout(() => {
                            game.musik2.play()
                        }, 6000);
                        tambahScore(100)
